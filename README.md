...
Status: versão 0.4.0, núcleo, comando de criar sistema, regras que consultam outras tabelas, usuário seguro com login pelo e-mail, telas de login com primeiro acesso seguro, proteção contra força bruta, verificação em duas etapas, páginas de erro em português e importar/exportar planilha na tela de banco, arquivos privados e compartilhados e histórico automático de todas as tabelas. Ainda não usar em produção (faltam registro de acessos, monitor de erros, jobs e o portão de deploy).
...
```
uvx --from "git+https://github.com/freitas260710/infra-vibecoding@v0.4.0" infra-vibecoding novo-sistema NOME
```
...
```
uv add "infra-vibecoding @ git+https://github.com/freitas260710/infra-vibecoding@v0.4.0"
```
